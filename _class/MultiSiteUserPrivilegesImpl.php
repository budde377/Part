 ...
 * Time: 17:17
 */
class MultiSiteUserPrivilegesImpl implements MultiSiteUserPrivileges
{
    private $user;
    private $siteLibrary;
    private $connection;
    ...
    {
        return $this->addPrivilege(MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT);
    }
    ...
    {
        return $this->addPrivilege(MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE, $site);
    }
    ...
    {
        return $this->addPrivilege(MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE, $site, $page);
    }
    ...
    public function revokePrivilege($site = null, $page = null)
    {
        ...
        if ($site == null && $page == null) {
            if (isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT])) {
                if ($this->deleteRootPreparedStatement == null) {
                    $this->deleteRootPreparedStatement = $this->connection->prepare("DELETE FROM MultiSiteUserPrivilege
                    WHERE username = ? AND type=?");
                }
                $this->deleteRootPreparedStatement->execute(array($this->user->getUsername(),
                    MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT));

                unset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT]);
                $nevPrivileges = array();

                foreach ($this->privileges as $privilege) {
                    if ($privilege['type'] != MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT) {
            ...
            if (isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE][$site])) {

                if ($this->deleteSitePreparedStatement == null) {
                    $this->deleteSitePreparedStatement = $this->connection->prepare("DELETE FROM MultiSiteUserPrivilege
                    WHERE username = ?  AND site = ? AND type=?");
                }

                $this->deleteSitePreparedStatement->execute(array($this->user->getUsername(), $site,
                    MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE));

                unset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE][$site]);
                $nevPrivileges = array();

                foreach ($this->privileges as $privilege) {
                    if ($privilege['type'] != MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE || $privilege['site'] != $site) {
                        $nevPrivileges[] = $privilege;
                    }
                }
                $this->privileges = $nevPrivileges;
            ...
            if (isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE][$site][$page])) {

                if ($this->deletePagePreparedStatement == null) {
                    $this->deletePagePreparedStatement = $this->connection->prepare("DELETE FROM MultiSiteUserPrivilege
                    WHERE username = ?  AND site = ? AND page = ? AND type=?");
                }

                $this->deletePagePreparedStatement->execute(array($this->user->getUsername(), $site,$page,
                    MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE));

                unset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE][$site][$page]);
                $nevPrivileges = array();

                foreach ($this->privileges as $privilege) {
                    if ($privilege['type'] != MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE || $privilege['site'] != $site
                        || $privilege['page'] != $page
                    ) {
                        $nevPrivileges[] = $privilege;
                    }
    ...
    public function isPrivileged($site = null, $page = null)
    {
        ...
        if ($site == null && $page == null) {
            return isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT]);
        } else if ($page == null) {
            return isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE][$site]) || $this->isPrivileged();
        } else if ($site != null) {
            return isset($this->overrideMap[MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE][$site][$page]) || $this->isPrivileged($site);
        }
        return false;
    }
    ...
    private function addPrivilege($type, $site = null, $page = null, $insertIntoDB = true)
    {
        ...
        if (($type == MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE || $type == MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE)) {
            $s = $this->siteLibrary->getSite($site);
            if ($s == null) {
                return false;
            }

            if ($type == MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE) {
        ...
        } else if ($type == MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT) {
        ...
        if ($insertIntoDB) {
            if ($this->addPrivilegePreparedStatement == null) {
                $this->addPrivilegePreparedStatement = $this->connection->prepare("INSERT INTO MultiSiteUserPrivilege (page,site,type,username) VALUES (?,?,?,?)");
            }
    ...
    private function initializePrivileges()
    {
        if (!$this->isInitialized) {
            ...
            $statement = $this->connection->prepare("SELECT * FROM MultiSiteUserPrivilege WHERE username = ?");
            $statement->execute(array($this->user->getUsername()));
            $rows = $statement->fetchAll(PDO::FETCH_ASSOC);
            foreach ($rows as $row) {
                $this->addPrivilege($row['type'], $row['site'], $row['page'], false);
                switch ($row['type']) {
                    case MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_ROOT:
                        $this->overrideMap[$row['type']] = 1;
                        break;
                    case MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_SITE:
                        $this->overrideMap[$row['type']][$row['site']] = 1;
                        break;
                    case MultiSiteUserPrivileges::USER_PRIVILEGES_TYPE_PAGE:
    ...
    public function revokeAllPrivileges()
    {
        if($this->deleteAllPreparedStatement == null){
            $this->deleteAllPreparedStatement = $this->connection->prepare("DELETE FROM MultiSiteUserPrivilege WHERE username=?");
        }
